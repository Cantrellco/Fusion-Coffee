 ...
 *
 * Leave the secret blank and the Fusion half quietly does nothing; everything
 * Little Town does carries on untouched. The value is PARTY_FEED_SECRET in
 * Cloudflare Pages → fusion-coffee → Settings → Environment variables.
 *
 * ⚠️ This feed returns customer names and phone numbers, so the secret is a
 * credential. It is why the feed refuses every request when it isn't set,
 * rather than falling open.
 */
var FUSION_FEED_URL = 'https://www.fusioncoffeeshop.com/api/party-bookings';
var FUSION_FEED_SECRET = '';
...
function upsertEvent_(p) {
  ...
  // one of them on its next run. See the warning on that function.
  if (p.venue === 'fusion') {
...
/**
 * Which colour this party gets. A booking with no package recorded goes grey
 * rather than being guessed at as plain Little Town — grey means "look this one
 * up in Shopify".
 */
 ...
 *
 * Safe to run repeatedly: the ids come from date + time, so a second run
 * updates the same events rather than duplicating.
 */
...
function runFusionSelfTest() {
  if (!FUSION_FEED_SECRET) {
    Logger.log('❌ FUSION_FEED_SECRET is empty — set it first.');
    return;
  }
  ...
  Logger.log(
    colourOk && noSlotTag
      ? '✅ PASS — feed reachable, event green, and no ltSlot tag so prune will leave it alone.'
